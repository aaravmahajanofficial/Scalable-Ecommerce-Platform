...
func NewReadinessHandler(cfg *config.Config, healthEndpoint *Endpoint) (http.Handler, error) {
	h, err := health.New(
		...
		health.WithChecks(
			newDatabaseCheck(cfg.Database.GetDSN()),
			newRedisCheck(cfg.RedisConnect.GetDSN()),
...
func newDatabaseCheck(dsn string) health.Config {
...
func newRedisCheck(dsn string) health.Config {
	return health.Config{
		Name:      "redis",
...
func newStripeCheck(healthEndpoint *Endpoint) health.Config {
	return health.Config{
		Name:      "stripe",
		Timeout:   5 * time.Second,
		SkipOnErr: false,
		Check:     checkStripe(healthEndpoint),
	}
}

func checkStripe(healthEndpoint *Endpoint) func(ctx context.Context) error {
